<template>
	<view class="container" @tap="togglePicker(0)">
					...
					<view v-else class="item flex-row" :class="[item.source == fromUserId ? 'right' : 'left']">
						...
						<view v-if="item.msgType == 'map'" class="content map" @tap="openLocation(item.content)">
							...
							<map class="box"
								...
								:markers="[{latitude: item.content.latitude, longitude: item.content.longitude, iconPath: '@/static/logo.png'}]"
								@tap="openLocation(item.content)"
							></map>
						</view>
						<!--语音-->
						<view v-if="item.msgType == 'voice'" @tap="playVoice(item)" class="content">
							<image v-show="voicePlayingId != item.id" src="@/static/logo.png" class="voice-icon"></image>
							<image v-show="voicePlayingId == item.id" src="@/static/logo.png" class="voice-icon"></image>
						</view>
					</view>
				</view>
		...
		<view class="oper flex-row" @tap.prevent.stop="">
			<!--语音切换-->
			<image v-if="isEdit" @tap="toggleEdit(false)" src="@/static/logo.png" class="icon"></image>
			<image v-else @tap="toggleEdit(true)" src="@/static/logo.png" class="icon"></image>
			<!--语音切换-->
			...
			<!--表情-->
			<image @tap="togglePicker(200, 'emoji')" src="@/static/logo.png" class="icon"></image>
			<!--发送-->
			<view @touchend.prevent="send" v-show="content" class="btn">发送</view>
			<!--附件-->
			<image @tap="togglePicker(86, 'file')" v-show="!content" src="@/static/logo.png" class="icon"></image>
		</view>
		<!--表情-->
